...
class TypedModelMetaclass(ModelBase):
    ...
    def __new__(mcs, class_name, bases, class_dict):
        ...
        if class_name == 'NewBase':
            return super(TypedModelMetaclass, mcs).__new__(
                mcs, class_name, bases, class_dict)
        try:
            TypedModel
        except NameError:
            ...
            typed_model = super(TypedModelMetaclass, mcs).__new__(mcs, class_name, bases, class_dict)
        ...
        if base_class:
            ...
            if getattr(Meta, 'proxy', False):
                ...
                return super(TypedModelMetaclass, mcs).__new__(mcs, class_name, bases, class_dict)
            Meta.proxy = True

            declared_fields = dict((name, element) for name, element in class_dict.items() if isinstance(element, Field))
            ...
            for field_name, field in declared_fields.items():
                ...
                if isinstance(field, models.fields.related.RelatedField):
                    ...
                    def do_related_class(self, other, cls):
                        ...
                        cls._meta.model_name = class_name.lower()
        ...
        class_dict['base_class'] = base_class

        cls = super(TypedModelMetaclass, mcs).__new__(mcs, class_name, bases, class_dict)
        ...
        if base_class:
            ...
            mcs._patch_fields_cache(cls, base_class)
        else:
