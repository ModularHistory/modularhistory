...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const getRedirectPath = async (path: string) => {
    let redirectPath = "";
    await axios
      .get(`http://${process.env.DJANGO_HOST}:${process.env.DJANGO_PORT}/api/redirects//${path}/`)
      .then((response) => {
  ...
  const retrieveFlatPage = async (path: string) => {
    ...
    await axios
      ...
      .get(`http://${process.env.DJANGO_HOST}:${process.env.DJANGO_PORT}/api/flatpages//${path}/`)
      .then((response) => {
