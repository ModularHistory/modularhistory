...
class ModerationAdminTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.admin = ChangeSetAdmin(Change, site)

        for user in User.objects.all():
            Change(content_object=user).save()
    ...
    def test_content_object_returns_deserialized_object(self):
        ...
        moderation = Change(content_object=user)
        moderation.save()
        content_object = self.admin.content_object(moderation)
        self.assertEqual(content_object, 'admin')

    def test_get_moderation_form(self):
        form = self.admin.get_moderation_form(UserProfile)
        self.assertIn('ModerationForm', repr(form))


class ModerationAdminBehaviorTestCase(WebTestCase):
    fixtures = ['test_users.json']
    ...
    def test_set_changed_by_property(self):
        ...
        self.assertEquals(self.book.moderation.changed_by, None)
        ...
        book = Book.unmoderations.get(pk=self.book.pk)  # refetch the obj
        self.assertEquals(book.title, 'Book not modified')
        moderated_obj = Change.objects.get_for_instance(book)
...
class AdminActionsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.moderations = Change.objects.all()

    def tearDown(self):
        teardown_moderation()

    def test_approve_objects(self):
        approve_objects(self.admin, self.request, self.moderations)

        for obj in Change.objects.all():
            self.assertEqual(obj.status, ModerationStatus.APPROVED)

    def test_reject_objects(self):
        qs = Change.objects.all()

        reject_objects(self.admin, self.request, qs)

        for obj in Change.objects.all():
            self.assertEqual(obj.status, ModerationStatus.REJECTED)

    def test_set_objects_as_pending(self):
        for obj in self.moderations:
            obj.approve(by=self.request.user)

        set_objects_as_pending(self.admin, self.request, self.moderations)

        for obj in Change.objects.all():
            self.assertEqual(obj.status, ModerationStatus.PENDING)
...
class ModerationAdminSendMessageTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.admin = ChangeSetAdmin(UserProfile, site)

        self.profile = UserProfile.objects.get(user__username='moderator')
        self.moderated_obj = Change(content_object=self.profile)
        self.moderated_obj.save()

    def tearDown(self):
        teardown_moderation()

    def test_send_message_when_object_has_no_moderation(self):
...
try:
    from apps.moderation.filterspecs import ContentTypeFilterSpec
except ImportError:
    # Django 1.4
    pass
else:
    ...
    class ContentTypeFilterSpecTextCase(TestCase):
        ...
        def test_content_types_and_its_order(self):
            f = Change._meta.get_field('content_type')
            filter_spec = ContentTypeFilterSpec(f, self.request, {}, Change, self.admin)
